...
ncu_sequence * ncu_sequence_init(list_sequence * listSeq,int startX, int startY,int sizeX,int sizeY){
	...
	this->inpStr = ncu_inputString_init(startX,startY+sizeY-1,10); 
	return this;
}
...
void  ncu_sequence_new_elem(ncu_sequence * this,ncu_cmdLine * cmda){
	...
	for(int i = 0;i<11;i++)
		name[i]= ' ';
	name[10]='\0';
	ncu_inputString_changeCible(this->inpStr,name,10);
	mvwin(this->inpStr->win,y+this->seq->nb_total+2,x+2);
	ncu_inputString_action(this->inpStr,cmda);	

	sequence_add(this->seq,SEQUENCE_AT_END,name,this->seq->nb_total,NULL);	
...
void ncu_sequence_dest(ncu_sequence * this){
	delwin(this->win);
	ncu_inputString_dest(this->inpStr);	
	free(this);	
} 
